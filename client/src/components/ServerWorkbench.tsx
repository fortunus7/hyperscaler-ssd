...
import { BadgeCheck, ChevronRight, CircleAlert, FileDown, FileUp, Loader2, Save, ServerCog, ShieldCheck, Trash2 } from "lucide-react";
...
export function ServerWorkbench() {
  ...
  const createMutation = trpc.serverCandidate.create.useMutation({ onSuccess: async () => { setDraft(blankDraft()); setMessage("서버 후보를 저장했습니다."); await utils.serverCandidate.list.invalidate(); }, onError: (mutationError) => setError(mutationError.message) });
  const importMutation = trpc.serverCandidate.importMany.useMutation({ onSuccess: async (result) => { setMessage(`${result.imported}개 서버 후보를 저장했습니다.`); await utils.serverCandidate.list.invalidate(); }, onError: (mutationError) => setError(mutationError.message) });
  const deleteMutation = trpc.serverCandidate.delete.useMutation({ onSuccess: async () => { setMessage("서버 후보를 삭제했습니다."); await utils.serverCandidate.list.invalidate(); }, onError: (mutationError) => setError(mutationError.message) });
  useEffect(() => { if (activeServerId === null && serversQuery.data?.[0]) setActiveServerId(serversQuery.data[0].id); }, [activeServerId, serversQuery.data]);
  const activeServer = serversQuery.data?.find((server) => server.id === activeServerId) ?? serversQuery.data?.[0]; const targetProfile = profiles.find((profile) => profile.id === targetProfileId) ?? profiles[0];
  ...
  const save = () => { setMessage(null); setError(null); if (!draft.manufacturer.trim() || !draft.model.trim()) { setError("서버 제조사와 모델명은 필수입니다."); return; } createMutation.mutate(toInput(draft)); };
  const handleFile = async (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setMessage(null); setError(null); try { const parsed = parseServerCsv(await file.text(), file.name); importMutation.mutate({ candidates: parsed.map(toInput) }); } catch (parseError) { setError(parseError instanceof Error ? parseError.message : "파일을 해석하지 못했습니다."); } event.target.value = ""; };
  ...
  return <section id="servers" className="border-y border-[#17202A]/12 bg-[#EAE5DB]/55 px-4 py-10 sm:px-7 lg:px-10 lg:py-12"><div className="mx-auto max-w-[1380px]"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="font-mono-ui text-[10px] font-medium tracking-[0.16em] text-[#E65B32]">04 · SERVER PROFILE WORKBENCH</p><h2 className="font-display mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">서버와 SSD의 경계를 함께 봅니다.</h2></div><p className="max-w-xl text-xs leading-5 text-[#6B6D69]">랙 크기, 전력 용량, CPU·메모리·네트워크, SSD 폼팩터·인터페이스·프로토콜·개수·용량·성능을 내부 후보 서버와 동일한 축에서 관리합니다.</p></div>
    {!authLoading && !isAuthenticated ? <div className="mt-6 grid gap-5 border border-[#17202A]/14 bg-[#FFFDF8] p-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"><div><div className="flex items-center gap-2 text-[#E65B32]"><ServerCog className="h-5 w-5" /><span className="font-mono-ui text-[10px] tracking-[.12em]">PRIVATE SERVER LIBRARY</span></div><h3 className="font-display mt-3 text-xl font-semibold">내부 서버 후보는 로그인 후에만 저장·열람됩니다.</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#59636B]">물리 서버의 랙·전력 사양과 SSD 베이 구성을 사용자 계정에 보존합니다. 공개 클라우드 인스턴스의 비공개 물리 호스트와 동일하다고 가정하지 않습니다.</p></div><button onClick={() => startLogin({ force: true })} className="inline-flex h-11 items-center justify-center gap-2 rounded-sm bg-[#17202A] px-5 text-sm font-semibold text-white transition hover:bg-[#243241] active:scale-[.98]">서버 후보 시작하기 <ChevronRight className="h-4 w-4 text-[#E65B32]" /></button></div> : <>
